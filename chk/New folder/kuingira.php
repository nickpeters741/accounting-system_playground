...
if (isset($_REQUEST['cabi'])) {
	if (empty($_REQUEST['cabi'])) { 
		...
		$shop = mysqli_real_escape_string($connection,$_REQUEST['shop']);
		...
		if ($login_status!=2) {
			...
			if ($work_status!=2) {
				...
				if (mysqli_num_rows($verify) < 1) {
					...
					<script type="text/javascript">
						var hulla = new hullabaloo();
					    hulla.send('YOU ARE NOT ASSIGNED ANY ROLE!!!!!CONTACT ADMIN', "danger"); 
					</script>
					<?php
				}else{
					...
					if (!$create_logger_details) {
						...
						if (!$update_login_status) {
							...
							$get_logo = mysqli_query($connection,"SELECT image_path FROM  kp_settings_gen") or die("Could not get the gen details ".mysqli_error($connection));
							$gen = mysqli_fetch_assoc($get_logo);
							$_SESSION['cust_logo'] = $gen['image_path'];
							//$_SESSION['dash'] = $dash;

							$get_thesion = mysqli_query($connection,"SELECT * FROM kp_thesion WHERE user_id='$user_id'") or die("Could not get the user permissions");
						    $num = mysqli_num_rows($get_thesion);
						    if ($num<1) { 
						    	thesion_init($user_id,$dash,$panel);
						    }else{ }
							if ($num>=1) { 
