...
function track_stock($item_id,$sale_id,$qty,$stock_bal,$day,$uom){
	global $connection,$user_id;
	$check_track= "SELECT * FROM kp_stock_track WHERE item_id='$item_id' AND sale_id='$sale_id' AND uom='$uom'";
		...
		if (mysqli_num_rows($check_track_r)==0) {
	    	$tracking = mysqli_query($connection,"INSERT INTO kp_stock_track(item_id,sale_id,user_id,qty,status,day,stock_bal,uom) VALUES('$item_id','$sale_id','$user_id','$qty','OUT','$day','$stock_bal','$uom')") or die("Error stock tracking". mysqli_error($connection));
	    }else{
	    	$update_track = "UPDATE kp_stock_track SET qty=qty+'$qty', stock_bal='$stock_bal' WHERE item_id='$item_id' AND sale_id='$sale_id' AND uom='$uom'";
...
function make_sale($item_id,$sale_id,$day,$user_id,$price,$qty,$sale_type,$uom){
	global $connection;
	...
	if ($portion==1) { 
		...
		while ($row = mysqli_fetch_assoc($get_portion_items)){
		    $raw_id = $row['raw_id'];
		    $raw_qty = $row['qty'];
		    $update_item_stock = mysqli_query($connection,"UPDATE kp_items SET qty=qty-'$raw_qty',$uomqty=$uomqty-'$raw_qty' WHERE item_id='$raw_id'") or die(mysqli_error($connection));
		    track_stock($item_id,$sale_id,$raw_qty,$stock_bal,$day,$uom);
		}  
	}else{
		$update_item_stock = mysqli_query($connection,"UPDATE kp_items SET qty=qty-'$qty',$uomqty=$uomqty-'$qty'  WHERE item_id='$item_id'") or die(mysqli_error($connection));
		track_stock($item_id,$sale_id,$qty,$stock_bal,$day,$uom);
	}

	$get_order= "SELECT * FROM kp_sale_items WHERE item_id='$item_id' AND sale_id='$sale_id' AND uom='$uom'";
	$get_order_r = mysqli_query($connection,$get_order) or die("Could not get the product price");
    if (mysqli_num_rows($get_order_r)==0) {
    	$add_order = "INSERT INTO kp_sale_items(sale_id,item_id,qty,price,total,day,profit,commission,type,bprice,uom,tax,tax_id)  VALUES('$sale_id','$item_id','$qty','$sprice','$total','$day','$profit','$commission','$sale_type','$bprice','$uom','$tax_amount','$tax_id')";
		$add_order_r = mysqli_query($connection,$add_order) or die("Could not add order". mysqli_error($connection));
		 
    }else{
        $update_order = "UPDATE kp_sale_items SET qty=qty+'$qty', total=qty*price,tax=('$tax'/'$q')*total  WHERE item_id='$item_id' AND sale_id='$sale_id' AND uom='$uom'";
