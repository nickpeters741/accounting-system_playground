...
function track_stock($item_id,$sale_id,$qty,$stock_bal,$day){
	...
	$check_track= "SELECT * FROM kp_stock_track WHERE item_id='$item_id' AND sale_id='$sale_id' AND shop='$shop'";
		...
		if (mysqli_num_rows($check_track_r)==0) {
	    	$tracking = mysqli_query($connection,"INSERT INTO kp_stock_track(item_id,sale_id,user_id,qty,status,day,stock_bal,shop) VALUES('$item_id','$sale_id','$user_id','$qty','OUT','$day','$stock_bal','$shop')") or die("Error stock tracking". mysqli_error($connection));
	    }else{
	    	$update_track = "UPDATE kp_stock_track SET qty=qty+'$qty',stock_bal='$stock_bal',shop='$shop' WHERE item_id='$item_id' AND sale_id='$sale_id'";
...
function make_sale($item_id,$sale_id,$day,$user_id,$price,$qty,$sale_type){
	...
	$get_order= "SELECT * FROM kp_sale_items WHERE item_id='$item_id' AND sale_id='$sale_id' AND shop='$shop'";
	$get_order_r = mysqli_query($connection,$get_order) or die("Could not get the product price");
    if (mysqli_num_rows($get_order_r)==0) {
    	$add_order = "INSERT INTO kp_sale_items(sale_id,item_id,qty,price,total,day,profit,commission,type,bprice,tax,tax_id,shop)  VALUES('$sale_id','$item_id','$qty','$sprice','$total','$day','$profit','$commission','$sale_type','$bprice','$tax_amount','$tax_id','$shop')";
