  ...
  $( "#bar" ).autocomplete({
    ...
    if (samount==0) {
      ...
      hulla.send('Warning!!! Sale Cannot be Empty', "warning"); 
	    ...
	    $.ajax({
	      	url: 'check_out.php',
	     	typr: 'POST',
	      	data: 'deli='+deli+'&reg='+reg+'&beba_sta='+beba_sta+'&means='+means+'&disc='+disc+'&vatable='+vatable+'&vatt='+vatt+'&tax='+tax,
	      	dataType: 'html'
	    })
	    .done(function(data){ 
	      	$('#load').html(''); 
    ...
    if (samount==0) {
  		...
  		$.ajax({
	      	...
	      	typr: 'POST',
	      	data: {deli:deli,lporefno:lporefno,delino:delino,reg:reg,beba_sta:beba_sta,means:means,vatable:vatable,vatt:vatt,tax:tax},
	      	dataType: 'html'
	    })
	    .done(function(data){
	      	//console.log(data);
