...
if (empty($qty)) {
	...
	$total = $qty * $price;
	$update_order = "UPDATE kp_sale_items SET qty='$qty' WHERE item_id='$item_id' AND sale_id='$sale_id'";
    $update_order_r = mysqli_query($connection,$update_order) or die("Could not create"); 

    $update_tt = "UPDATE kp_sale_items SET total=qty*price,profit=profit*'$qty', tax=('$tax'/'$q')*total WHERE item_id='$item_id' AND sale_id='$sale_id'";
    $update_tt_r = mysqli_query($connection,$update_tt) or die("Could not create"); 

    $update_track = "UPDATE kp_stock_track SET qty='$qty',stock_bal='$stock_bal' WHERE item_id='$item_id' AND sale_id='$sale_id'";
    $update_track_r = mysqli_query($connection,$update_track) or die("Could not create");    

    
	$update_store = mysqli_query($connection,"UPDATE kp_items SET qty='$stock_bal' WHERE item_id='$item_id'");
