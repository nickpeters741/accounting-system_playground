...
if ($st=="3") {
	...
	update_thesion("sup_id",$sup_id,$page);
	$action="CONTINUED LPO NO| ".$po_id;
	$url="../../";
	header("Location: new_po.php");
	?>
	<script type="text/javascript">
		window.location = 'new_po.php';  
	</script>
	<?php
}else{
	$check_id=mysqli_query($connection,"SELECT po_id FROM kp_po_status WHERE  po_user_id='$user_id' AND sup_id='$sup_id' AND po_status=2");
	if (mysqli_num_rows($check_id) > 0) {
		...
		$create_po_status = "INSERT INTO kp_po_status(po_id,po_user_id,sup_id) VALUES('$po_id','$user_id','$sup_id')";
