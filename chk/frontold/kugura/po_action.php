...
if (empty($item_id) OR $item_id==0) {
	...
	$results = mysqli_query($connection ,"SELECT * FROM kp_po_items WHERE item_id ='$item_id' AND po_id='$po_id' AND shop='$shop'") or die("error selectig po items".mysqli_error($connection));
	if(mysqli_num_rows($results) < 1) {
		$create = mysqli_query($connection,"INSERT INTO kp_po_items(po_id,item_id,qty,price,total,day,shop) VALUES('$po_id','$item_id','$quantity','$price','$amount',CURRENT_DATE,'$shop')") or die(mysqli_error($connection));
		if (!$create) {
