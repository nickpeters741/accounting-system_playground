...
function add_item(){
    global $connection,$item_id,$lpo_id,$day,$user_id,$price,$qty,$shop;
    ...
    $total=$qty*$sprice;
    $get_order= "SELECT * FROM kp_lpo_items WHERE item_id='$item_id' AND lpo_id='$lpo_id' AND shop='$shop'";
    $get_order_r = mysqli_query($connection,$get_order) or die("Could not get the product price");
    if (mysqli_num_rows($get_order_r)==0) {
        $add_order = "INSERT INTO kp_lpo_items(lpo_id,cust_id,item_id,qty,price,total,day,shop)  VALUES('$lpo_id','$cust_id','$item_id','$qty','$sprice','$total','$day','$shop')";
