...
$check_item_no = mysqli_query($connection,"SELECT qty,item_id,sale_id,uom FROM kp_sale_items WHERE id='$id'") or die(mysqli_error($connection));
$q = mysqli_fetch_assoc($check_item_no);
$qty = $q['qty'];
$item_id = $q['item_id'];
$sale_id = $q['sale_id'];
$uom_id = $q['uom']; 
$uomqty="uom".$uom_id."qty";
$update_store = mysqli_query($connection,"UPDATE kp_items SET qty=qty+'$qty',$uomqty=$uomqty+'$qty' WHERE item_id='$item_id'") or die(mysqli_error($connection));
