...
if (empty($item_id) OR empty($quantity)) {
	...
	$results = mysqli_query($connection ,"SELECT * FROM kp_qt_items WHERE item_id ='$item_id' AND qt_id='$qt_id' AND shop='$shop'") or die(mysqli_error($connection));
	if(mysqli_num_rows($results) < 1) {
		$create = mysqli_query($connection,"INSERT INTO kp_qt_items(qt_id,item_id,qty,price,total,shop) VALUES('$qt_id','$item_id','$quantity','$price','$amount','$shop')") or die(mysqli_error($connection));
		if (!$create) {
			?>
			<div class="panel-body">
				<div class="row">
