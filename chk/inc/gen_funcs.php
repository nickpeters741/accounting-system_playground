...
function  page_restrict(){
	...
	if (!isset($_SESSION['user_id']) OR empty($_SESSION['user_id']) OR $_SESSION['user_id']==0) {
...
///
//ids///
function invoice_id($shop){ 
	global $connection;
	$invoiceid=mysqli_query($connection,"SELECT * FROM kp_invoiceids where shop ='$shop' order by id DESC limit 1 ") or die("Error".mysqli_error($connection));
	    $rowie = mysqli_fetch_assoc($invoiceid);
    if (is_null($rowie['invoiceid'])) {
    	$invoice_no =0;
    }else{
    	$invoice_no = $rowie['invoiceid'];
    }
	return $invoice_no;
...
function thesion_init($user_id,$dash,$panel,$shop){
	global $connection; 
	$mac_address=get_mac();
	...
	$create_thesion= mysqli_query($connection,"INSERT INTO kp_thesion(user_id,dashboard,panel,comp_address,day,shop) VALUES('$user_id','$dash','$panel','$mac_address',CURRENT_DATE,'$shop') ON DUPLICATE KEY  UPDATE   dashboard='$dash',panel='$panel',comp_address='$mac_address',shop='$shop'") or die(mysqli_error($connection));
    return true;
}
...
//// CURRENT STOCK QTY ////// 
function get_currentstock($item_id){
	global $connection;
	$get_currentstock= mysqli_query($connection,"SELECT qty FROM kp_production_items WHERE item_id='$item_id'");
	$row = mysqli_fetch_assoc($get_currentstock);
	$qty = strtoupper($row['qty']);
	return $qty;
}
//// EXPENSE ////// 
function get_expensetype($expense){
	global $connection;
	$get_expense= mysqli_query($connection,"SELECT * FROM kp_expensetypes WHERE id='$expense'");
	$row = mysqli_fetch_assoc($get_expense);
